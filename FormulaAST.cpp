...
namespace ASTImpl {
    ...
    public:
        ...
        virtual CellInterface::Value Evaluate(std::function<CellInterface::Value(const Position&)>&) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            CellInterface::Value Evaluate(std::function<CellInterface::Value(const Position&)>& f) const override {
                if (type_ == Type::Add) {
                    return std::get<double>(lhs_->Evaluate(f)) + std::get<double>(rhs_->Evaluate(f));
                }
                    ...
                    if (std::get<double>(rhs_->Evaluate(f)) != 0) {
                        return std::get<double>(lhs_->Evaluate(f)) / std::get<double>(rhs_->Evaluate(f));
                    }
                    else {
        ...
        public:
            ...
            CellInterface::Value Evaluate(std::function<CellInterface::Value(const Position&)>& f) const override {
                if (type_ == Type::UnaryMinus) {
                    return -(std::get<double>(operand_->Evaluate(f)));
                }
        ...
        public:
            ...
            CellInterface::Value Evaluate(std::function<CellInterface::Value(const Position&)>& f) const override {
                ...
                if (std::holds_alternative<std::string>(result)) {
                    ...
                    if (is_number(std::get<std::string>(result))) {
                        return std::stod(std::get<std::string>(result));
                    }
                    else {
                        return CellInterface::Value(FormulaError::Category::Value);
                    }
                }
                return CellInterface::Value(FormulaError::Category::Value);
            }
...
CellInterface::Value FormulaAST::Execute(std::function<CellInterface::Value(const Position&)>& f) const {
    return root_expr_->Evaluate(f);
}
